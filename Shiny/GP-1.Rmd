...
server <- function(input, output, session) {
  ...
  observe({
    lapply(1:4, function(i) shinyjs::removeClass(paste0("step", i, "_box"), "step-active"))
    ...
    output$stage_ui <- renderUI({
      switch(step(),
        ...
        div(
          ...
          fluidRow(
            column(6, h5("Active Heatmap"), imageOutput("heatmap_img", width = "300px", height = "300px")),
            column(6, h5("Outline"), imageOutput("contour_img", width = "300px", height = "300px"))
          ),
  ...
  observeEvent(input$img_file, {
    req(input$img_file)
    if (file.exists("image") && !dir.exists("image")) file.remove("image")
  ...
  observeEvent(input$run_denoiser, {
    req(img_path())
    input_img <- normalizePath("image/uploaded.jpg")
    output_img <- normalizePath("image/denoised.jpg")
    ...
    if (file.exists(output_img)) {
      ...
      output$denoiser_status <- renderText("Denoised image not created.")
    }
  })
  ...
  observeEvent(input$run_analysis, {
    ...
    Sys.sleep(1)
    input_img <- normalizePath("image/denoised.jpg", mustWork = FALSE)
    result_txt <- normalizePath("image/result.txt", mustWork = FALSE)
    result_img <- normalizePath("image/heatmap.png", mustWork = FALSE)
    ...
    if (file.exists(rmd_model)) {
      rmarkdown::render(rmd_model, params = list(
        input_img = input_img,
        method = input$denoise_model,
        model_type = input$analysis_model,
        result_txt = result_txt,
        result_img = result_img
      ), envir = new.env(), knit_root_dir = getwd(), quiet = TRUE)
    }
    if (file.exists(result_txt)) {
      score_val <- as.numeric(readLines(result_txt, warn = FALSE))
    ...
    output$heatmap_img <- renderImage({
      list(src = "image/heatmap.png", contentType = "image/png", width = 300, height = 300)
    }, deleteFile = FALSE)
    output$contour_img <- renderImage({
      list(src = "image/contour.png", contentType = "image/png", width = 300, height = 300)
    }, deleteFile = FALSE)
    removeModal()
    shinyjs::show("next_step")
  })
