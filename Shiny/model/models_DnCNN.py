...
DNCNN_MODEL_PATH = "DnCNN_model/model_DnCNN_sigma=25_epoch_37.pth"

class DnCNN_Denoiser():
    def __init__(self):
        self.model = DnCNN(input_chnl=3)
        self.model.load_state_dict(torch.load(DNCNN_MODEL_PATH, weights_only=False), strict=False)
    ...
    def denoise_image(self, img: Image.Image)-> Image.Image:
        ...
        with torch.no_grad():
            ...
            denoised = torch.clamp(img_tensor - output, 0.0, 1.0).squeeze(0).cpu()
